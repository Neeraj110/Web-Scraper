...
const Bookmarks = () => {
  ...
  const bookmarkIds = useMemo(() => new Set((user?.bookmarks || []).map(String)), [user]);
  ...
  useEffect(() => {
    const loadBookmarks = async () => {
      ...
      try {
        ...
        const savedStories = (data.stories || []).filter((story) => bookmarkIds.has(String(story._id)));

        setStories(savedStories);
        setError('');
      } catch (requestError) {
        setError(requestError.response?.data?.message || 'Failed to load bookmarks');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleBookmark = async (storyId) => {
    ...
    try {
      ...
      setStories((currentStories) => currentStories.filter((story) => String(story._id) !== storyId));
    } finally {
  ...
  return (
    <div className="space-y-6 pb-12">
      <section className="rounded-[2rem] border border-black/8 bg-white/85 p-6 shadow-soft sm:p-8">
        <p className="text-xs font-bold uppercase tracking-[0.24em] text-ember">Your shelf</p>
        <h1 className="mt-3 font-serif text-4xl font-black tracking-tight text-ink">Saved stories</h1>
        <p className="mt-3 max-w-2xl text-ink/70">
          Bookmarked stories stay attached to your account and are removed automatically when the feed refreshes them away.
        </p>
      </section>
      ...
      {loading ? (
        ...
        <div className="rounded-[1.5rem] border border-black/8 bg-white/80 px-6 py-12 text-center shadow-soft">
          ...
          <p className="mt-2 text-ink/65">Open the home feed and save a few stories to build this list.</p>
        </div>
      ) : (
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {stories.map((story) => (
