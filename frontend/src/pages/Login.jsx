  ...
  return (
    <div className="mx-auto grid max-w-5xl gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
      <div className="rounded-[2rem] border border-black/8 bg-ink p-8 text-paper shadow-soft">
        <p className="text-xs font-bold uppercase tracking-[0.24em] text-paper/60">Welcome back</p>
        <h1 className="mt-4 font-serif text-4xl font-black tracking-tight">Sign in to save stories.</h1>
        <p className="mt-4 max-w-xl text-paper/75">
          Your bookmarks are synced with the backend, so you can move across devices without losing your saved reads.
        </p>
        <div className="mt-8 rounded-[1.5rem] bg-white/10 p-5 ring-1 ring-white/10">
          ...
          <Link to="/register" className="mt-2 inline-block text-sm font-bold text-gold hover:underline">
            Create an account
          </Link>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-[2rem] border border-black/8 bg-white/90 p-8 shadow-soft">
        <h2 className="font-serif text-3xl font-black tracking-tight text-ink">Login</h2>
        <p className="mt-2 text-sm text-ink/65">Use the email and password you registered with.</p>
